...
class TrainListCtrl(wx.ListCtrl):
	...
	def ClearChecks(self):
		self.imgIndex = [self.idxEmpty for _ in range(len(self.trains))]
		self.refreshAll()
	...
	def refreshTrain(self, trName):
		for i in range(len(self.trains)):
			tr = self.trains[i]
			if tr["rname"] == trName:
				self.RefreshItem(i)

	def RemoveTrain(self, trName):
		for i in range(len(self.trains)):
			tr = self.trains[i]
			if tr["rname"] == trName:
				del self.trains[i]
	...
	def SelectAll(self):
		self.imgIndex = [self.idxSelected for _ in range(len(self.trains))]
		self.refreshAll()
		self.parent.reportSelection()
	...
	def GetChecked(self):
		return [self.trains[i]["rname"] for i in range(len(self.trains)) if self.imgIndex[i] == self.idxSelected]
